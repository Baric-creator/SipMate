...
export default function ChatScreen() {
  const { conversationId, name, id } = useLocalSearchParams<{
    ...
    const channel = supabase
      ...
      .on(
        ...
        (payload) => {
          ...
          if (
            typeof updatedProfile.avatar_url !==
            'undefined'
          ) {
            setOtherAvatar(
    ...
    const channel = supabase
      ...
      .on(
        'postgres_changes',
        {
          ...
          schema: 'public',
          table: 'messages',
          filter: `conversation_id=eq.${String(
            conversationId
          )}`,
        },
        (payload) => {
...
if (error) {
  ...
  if (
    ...
    if (typeof window !== 'undefined') {
      window.alert(
        "🚫 You can't send messages to this user because one of you has blocked the other."
      );
    }
