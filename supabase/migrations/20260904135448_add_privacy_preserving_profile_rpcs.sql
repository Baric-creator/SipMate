...
as $$
  with me as (
    ...
    select
      case
        when premium_active and custom_origin_latitude between -90 and 90 then custom_origin_latitude
        else latitude
      end as lat,
      case
        when premium_active and custom_origin_longitude between -180 and 180 then custom_origin_longitude
        else longitude
      end as lon
    from me
  ), candidates as (
    ...
    where auth.uid() is not null
      ...
      and not exists (
        ...
        where s.user_id = auth.uid()
          and s.skipped_user_id = p.id
      )
  )
  select c.id, c.name, c.age, c.city, c.bio, c.currently_up_for, c.is_active, c.avatar_url, c.gender, c.distance_km
  from candidates c
