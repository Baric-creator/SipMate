...
create or replace function public.enforce_profile_photo_limit() returns trigger language plpgsql as $$ begin return new; end $$;
create or replace function public.handle_new_user() returns trigger language plpgsql as $$ begin return new; end $$;
create or replace function public.handle_premium_subscription_change() returns trigger language plpgsql as $$ begin return coalesce(new,old); end $$;
create or replace function public.refresh_premium_offer_counts() returns void language plpgsql as $$ begin return; end $$;
create or replace function public.trigger_refresh_premium_offer_counts() returns trigger language plpgsql as $$ begin return coalesce(new,old); end $$;
create or replace function public.update_premium_offer_stage() returns void language plpgsql as $$ begin return; end $$;
