-- The atomic Stripe claim API migration is timestamped after the original caller
-- hardening migration and recreates these functions. Reassert the hardened
-- definitions here so a clean chronological replay cannot restore the historical
-- SECURITY DEFINER current_user authorization bug.
...
begin
  ...
  if v_status='processing'
     and v_last_attempt > now() - make_interval(secs => p_stale_after_seconds) then
    return false;
  end if;
