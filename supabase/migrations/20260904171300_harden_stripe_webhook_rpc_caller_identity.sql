-- Mirror the production hardening for Stripe webhook ledger RPCs.
-- SECURITY DEFINER makes current_user the function owner, so authorize using
-- the JWT role claim instead and keep direct EXECUTE service-role-only.
...
begin
  ...
  select status,last_attempt_at into v_status,v_last_attempt
  from public.stripe_webhook_events where event_id=p_event_id for update;
...
begin
  ...
  update public.stripe_webhook_events
  set status='processed',processed_at=now(),last_attempt_at=now(),last_error=null
  where event_id=p_event_id;
...
begin
  ...
  update public.stripe_webhook_events
  set status='failed',last_attempt_at=now(),last_error=left(coalesce(p_error,'unknown error'),2000)
  where event_id=p_event_id;
  if not found then raise exception 'unknown webhook event'; end if;
end;
$$;
