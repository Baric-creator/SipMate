...
if (exists('.env.example')) {
  ...
  const assignments = envExample.split(/\r?\n/).map((line) => line.trim()).filter((line) => line && !line.startsWith('#') && line.includes('='));
...
if (exists('supabase/config.toml')) {
  ...
  const functionJwtSetting = (name) => {
    const escaped = name.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
    const match = config.match(new RegExp(`\\[functions\\.${escaped}\\]([\\s\\S]*?)(?=\\n\\[functions\\.|$)`));
    return match?.[1]?.match(/verify_jwt\s*=\s*(true|false)/)?.[1] ?? null;
  };
  for (const fn of ['create-checkout-session', 'create-customer-portal', 'delete-account']) {
    assert(functionJwtSetting(fn) === 'true', `${fn} must keep platform JWT verification enabled`);
  }
  assert(functionJwtSetting('stripe-webhook') === 'false', 'Stripe webhook must remain callable without a user JWT so Stripe can deliver signed events');
}
